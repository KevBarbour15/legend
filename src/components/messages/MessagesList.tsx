...
const MessagesList: React.FC<MessagesListProps> = ({ updateUnreadCount }) => {
  ...
  const handleReadStatusChange = (messageId: string, newReadStatus: boolean) => {
    setMessages(prevMessages => {
      const updatedMessages = prevMessages.map(message =>
        message._id === messageId ? { ...message, read: newReadStatus } : message
      );
      
      // Calculate new unread count
      const unreadCount = updatedMessages.filter(message => !message.read).length;
      updateUnreadCount(unreadCount);
